...
import { getBookById } from '@/lib/api/openlibrary';
...
interface LibroDetailPageProps {
  params: Promise<{ id: string }>;
}

export async function generateMetadata({ params }: LibroDetailPageProps): Promise<Metadata> {
  const { id } = await params;
  const item = await getOrCacheItem(id, 'libro', () => getBookById(id));
  if (!item) return { title: 'Libro no encontrado' };
  return { title: item.title, description: item.description?.slice(0, 160) };
}

export default async function LibroDetailPage({ params }: LibroDetailPageProps) {
  const { id } = await params;
  const item = await getOrCacheItem(id, 'libro', () => getBookById(id));
  if (!item) notFound();
        ...
        <div className="space-y-4">
          <div>
            <div className="flex items-center gap-2 mb-2">
              <Badge variant="outline">Libro</Badge>
